...
using DataFrames, CSV, Plots, LaTeXStrings, BlackBoxOptim, Distances, Revise, Printf, Random
Revise.track("<path to diversity simulator>")
...
# import the cell data for that index
read_data = DataFrame(CSV.File("./data/big-agencies$(data_ind).csv"));
each_agency = groupby(read_data, :AGY);
import_data = [reverse(sort(each_agency[x][!,:EMPLOYMENT][:])) for x in 1:length(each_agency)];

Ns = floor.(sum.(import_data));
Ds = length.(import_data);

# pick out the smallest FA from the entire list
ag_all = DataFrame(CSV.File("./data/big-agencies.csv"));
all_agency = groupby(ag_all, :AGY)
all_rfs = [reverse(sort(all_agency[x][!,:EMPLOYMENT][:])) for x in 1:length(all_agency)];
    
small_fa = all_rfs[6]; 
# min size at 6 which is argmin(Ns)
...
function pad_rdist(d1, d2)
    ...
    d1_padded = vcat(d1, ones(n - length(d1))*min_not_0/3); # divide by 3, so as not to punish the distance too badly (could make things too stochastic).
    d2_padded = vcat(d2, ones(n - length(d2))*min_not_0/3);
...
Threads.@threads for iter in 1:nth
    ...
    for j in 1:runs_per_th[iter]
        ...
        initial_val_d = length(small_fa)
        initial_val_n = sum(small_fa)
        ...
        function ibb_fn(S::Array{Float64,1})
            ...
            if Qred == 1
                ...
                osim = dsim.create_cell(θ,γ,1.9563,convert(Int64,reduced_n), small_fa, :pdf)
                ...
                osim = dsim.create_cell(θ,γ,1.963,convert(Int64,Ns[i]), small_fa, :pdf)
...
df_out = DataFrame(theta=θ_vec_all, gamma=γ_vec_all);
CSV.write("./calibration_results/fa-calibs-results-$(data_ind).csv", df_out)
